...
const columns: GridColDef[] = [
    ...
    {
        field: 'startdate', headerName: 'תאריך התחלה', width: 105, valueGetter: (params: GridValueGetterParams) =>
            params.row.startdate ? dateFormat(new Date(params.row.startdate)) : '',
    },
    {
        field: 'enddate', headerName: 'תאריך סיום', width: 105, valueGetter: (params: GridValueGetterParams) =>
...
const MyDutiesTable = () => {
    ...
    useEffect(() => {
        const getPotentials = async () => {
            const response = await fetch("http://localhost:3000/potential/shifts/" + localStorage.getItem("SoldierID"));
    ...
    return (
        <div ref={usersTableContainer} style={{
            ...
            {
                rows.length ? <GenericTable pageSize={5} items={rows} columns={columns} /> : <CircularProgress/>
            }
        </div>
    )
}
